...
def print2():
    if enable.is_pressed:
        ...
        # Take initial photo when door opens
        filename = f"/var/www/html/images/{time.strftime('%Y-%m-%d_%H-%M-%S')}_image_1_from_door_open.jpg"
        log_and_print(f"Taking photo: {filename}")
        os.system(f"sudo fswebcam -r 1280x720 --no-banner {filename}")
    else:
    ...
    while True:
        if DoorOpen and enable.is_pressed:
            while i <= 10:
                ...
                print(i)
                if i == 10:
                    # Take repeated photo every 10 seconds
                    filename = f"/var/www/html/images/{time.strftime('%Y-%m-%d_%H-%M-%S')}_image_{b}_from_door_open.jpg"
                    log_and_print(f"Taking photo: {filename}")
                    os.system(f"sudo fswebcam -r 1280x720 --no-banner {filename}")
                    i = 1
                    b += 1
